"""
MED-Review Video Editor — Hugging Face Spaces
Usa Gradio pra interface web + medreview_engine.py pra processamento real.
"""
import gradio as gr
import os
import sys
...
def process_video(video, nome, name_sub, tema, duracao, legendas):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        # Monta argumentos como o CLI espera
        class Args:
            pass
        args = Args()
        args.input = video
        ...
        # Copia pra local permanente (Gradio precisa de path persistente)
        out_permanent = f"/tmp/output_{stem}{suffix}.mp4"
        shutil.copy2(args.output, out_permanent)
...
with gr.Blocks(
    ...
    gr.Markdown("# 🎬 MED-Review Video Editor\nEditor automático de depoimentos — transcreve, legenda e edita.")
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            namesub_input = gr.Textbox(label="Subtítulo", value="Aluno Med-Review",
                                       placeholder="Ex: Aprovada em Dermato")
            tema_input = gr.Dropdown(choices=list(THEMES.keys()), value="Experiência",
                                     label="Tema")
            duracao_input = gr.Dropdown(choices=list(DURACOES.keys()), value="Vídeo completo",
                                        label="Duração")
            legendas_check = gr.Checkbox(value=True, label="Gerar legendas automáticas")
            
            process_btn = gr.Button("🚀 Processar", variant="primary", size="lg")
        
        with gr.Column(scale=1):
            video_output = gr.Video(label="✅ Vídeo editado")
            gr.Markdown("*O processamento pode levar 1-3 minutos (transcrição + renderização)*")
